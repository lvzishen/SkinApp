...
public class VisibilityTracker {
    ...
    public VisibilityTracker(@NonNull final View rootView) {
        mTrackedViews = new WeakHashMap<View, TrackingInfo>(10);
        ...
        try {
            ...
            if (!viewTreeObserver.isAlive()) {
                if (DEBUG) {
                    Log.d(TAG, "Visibility Tracker was unable to track views because the"
            ...
            if (DEBUG) {
                Log.e(TAG, "#VisibilityTracker: Error " + e.getMessage());
            }
        }
    }

    VisibilityTracker(@NonNull final Context context, @NonNull final Map<View, TrackingInfo> trackedViews,
                      @NonNull final VisibilityChecker visibilityChecker, @NonNull final Handler visibilityHandler) {
        ...
        try {
            ...
            if (DEBUG) {
                Log.e(TAG, "#VisibilityTracker Constructor: Error " + e.getMessage());
            }
        }
    }

    public void setVisibilityTrackerListener(@Nullable final VisibilityTrackerListener visibilityTrackerListener) {
    ...
    public void addView(@NonNull View rootView, @NonNull final View view, final int minVisiblePercentageViewed,
                        final int maxInvisiblePercentageViewed) {
    ...
    public void startTracker() {
        ...
        try {
            if (mRootView != null && mRootView.get() != null) {
                final ViewTreeObserver viewTreeObserver = mRootView.get().getViewTreeObserver();
            ...
            if (DEBUG) {
                Log.e(TAG, "#startTracker: Error " + e.getMessage());
            }
        }
    }
